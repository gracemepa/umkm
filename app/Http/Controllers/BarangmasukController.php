...
if (!$barangmasuk) {
    return redirect()->route('admin.barangmasuk.index')->with('error', 'barangmasuk tidak ditemukan.');
}
...
if ($request->hasFile('gambar')) {
    ...
    if ($barangmasuk->gambar && file_exists(public_path($barangmasuk->gambar))) {
        unlink(public_path($barangmasuk->gambar)); // Hapus gambar lama
    }
}
...
{
    // Hapus data barang masuk dan gambar terkait
    $barangmasuk = Barangmasuk::find($id);

    if ($barangmasuk) {
        // Hapus gambar jika ada
        if ($barangmasuk->gambar && file_exists(public_path('uploads/barangmasuk/' . $barangmasuk->gambar))) {
            unlink(public_path('uploads/barangmasuk/' . $barangmasuk->gambar)); // Hapus gambar dari folder uploads/barangmasuk
        }
        $barangmasuk->delete();
